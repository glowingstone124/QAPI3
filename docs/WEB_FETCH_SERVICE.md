...
4. 服务不接收搜索关键词，只读取 URL。QAPI3 负责先通过 SearXNG 搜索，并按会话记录 15 分钟内的结果。Web 渠道可见 URL；QQ、Minecraft 等渠道只给模型不含 URL 的结果编号。`web_fetch` 在 QAPI3 内部把编号还原成 URL，再交给 `/fetch`，且不向非 Web 渠道返回 URL。
