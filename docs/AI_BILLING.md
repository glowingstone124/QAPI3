...
预估不能保证和供应商实际计费完全一致（尤其图片、工具新增上下文、搜索费用和缺失 Usage）。缺失 Usage 或数据库结算失败仍保留预留为 pending；账户设置读取和 Reset 卡兑换会自动幂等重试已有完整 Usage 的记录，也可通过 reconcile 接口重试，无需先充值。没有 Usage 且超过 30 分钟的 pending 请求会按失败退款并标记为 `refunded`，未超时的记录及未知供应商费用仍需运营核对；不要直接删除预留或自动按估值收费。
