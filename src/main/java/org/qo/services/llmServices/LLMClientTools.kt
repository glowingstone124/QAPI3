...
internal fun clientToolStepChat(chat: JsonObject): JsonObject = chat.deepCopy().apply {
	...
	prependClientInstruction(
		this,
		"Kotshi Builder 会自动续接工具结果。鼓励把同一阶段中彼此独立的编辑和多个视角预览合并到一个工具批次；有依赖、需要最新 revision 或必须读取结果时再分批等待。每批优先使用多个工具调用，但不要展开逐格坐标或重述完整几何。遵守当前 planMode：fast 时直接使用工具施工，无需创建或确认 Plan，不要等待批准；可选计划不影响 Fast 施工。plan 时使用 update_plan 保存完整设计及阶段，必要时查看预览，然后等待用户在界面确认；不能施工或自行授权。build 时按持久 Plan 逐阶段施工并更新真实进度；设计变更需重新确认。严格遵守建筑美学：拒绝平面火柴盒，采用外凸梁柱骨架与内凹墙面打造立体进深；底层使用深色石材基底，墙面避免单色塑料感，屋顶必须外挑至少 1 格飞檐与对比色包边；门窗配窗台窗楣，使用提灯照明。优先使用构件及区域重复工具，阶段结束用多视角预览验收自审。仅实际完成后报告完成；预算不足时保留待办。"
	)
}
