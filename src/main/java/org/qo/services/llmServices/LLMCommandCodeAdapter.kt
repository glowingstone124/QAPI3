    ...
    fun fromChatRequest(chat: JsonObject, tools: JsonArray, reasoningEffort: LLMReasoningEffort): JsonObject {
        ...
        chat.getAsJsonArray("messages").forEach { message ->
            ...
            when (role) {
                ...
                "assistant" -> {
                    val parts = JsonArray()
                    obj.get("reasoning_content")?.takeUnless { it.isJsonNull }?.asString?.takeIf { it.isNotBlank() }?.let {
                    ...
                    obj.getAsJsonArray("tool_calls")?.forEach { call ->
                        ...
                        val input = runCatching { JsonParser.parseString(function.get("arguments")?.asString ?: "{}").asJsonObject }
                    ...
                    messages.add(wireMessage("tool", JsonArray().apply {
                        add(JsonObject().apply {
                            ...
                            add("output", JsonObject().apply {
                                ...
                                addProperty("value", textContent(content)?.ifBlank { "(empty tool result)" } ?: "(empty tool result)")
                            })
                        })
                    }))
                }
        ...
        val maxTokens = listOf("max_completion_tokens", "max_tokens", "max_output_tokens")
            .firstNotNullOfOrNull { chat.get(it)?.takeUnless(JsonElement::isJsonNull)?.asInt?.takeIf { n -> n > 0 } } ?: 65536
        return JsonObject().apply {
            ...
            add("params", JsonObject().apply {
                ...
                if (reasoningEffort != LLMReasoningEffort.NONE) addProperty("reasoning_effort", reasoningEffort.wireValue)
            })
        ...
        content.asJsonArray.forEach { element ->
            ...
            when (part.get("type")?.asString) {
                ...
                "image_url", "input_image" -> {
                    ...
                    val match = Regex("^data:(image/[a-zA-Z0-9.+-]+);base64,([A-Za-z0-9+/=]+)$", RegexOption.IGNORE_CASE).matchEntire(url)
