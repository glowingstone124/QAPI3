...
interface AfdianOrderQuery {
	suspend fun query(orderNo: String): JsonObject
}

internal class AfdianOrderNotFoundException :
	IllegalStateException("Order not visible in the official query; retry required")
...
class HttpAfdianOrderQuery(
	...
	private val client =
		HttpClient(CIO) { install(HttpTimeout) { requestTimeoutMillis = 15_000; connectTimeoutMillis = 5_000 } }
...
class AfdianCreditsService @Autowired constructor(
	...
	suspend fun intent(uid: Long, amount: Int): JsonObject {
		...
		require(
			URI(pack.checkoutUrl).scheme == "https" && URI(pack.checkoutUrl).host in setOf(
				"ifdian.net",
				"afdian.com",
				"afdian.net"
			)
		...
		return JsonObject().apply {
			...
			addProperty(
				"checkout_url",
				pack.checkoutUrl + separator + "custom_order_id=" + URLEncoder.encode(id, Charsets.UTF_8)
			)
...
class AfdianCreditsController(private val llm: LLMServices, private val credits: AfdianCreditsService) {
	...
	suspend fun purchase(
		...
		return try {
			credits.intent(principal.qqUid, JsonParser.parseString(body).asJsonObject.get("amount").asInt).toString()
		} catch (_: IllegalArgumentException) {
			throw ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid credit pack")
		} catch (_: IllegalStateException) {
			throw ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "Payments not configured")
		}
	}
