            ...
            when (event.get("type")?.asString) {
                ...
                "tool-call" -> {
                    ...
                    calls.add(JsonObject().apply {
                        addProperty("id", event.get("toolCallId")?.asString?.takeIf { it.isNotBlank() } ?: "call_${UUID.randomUUID()}")
                ...
                "error" -> {
                    ...
                    result = commandCodeError(message ?: event.get("message")?.asString ?: "Command Code upstream error")
                    status = 502
                    return@execute
                }
            }
...
internal fun LLMServices.commandCodeFallbackRequest(request: LLMServices.NormalizedRequest, fallback: LLMProvider): LLMServices.NormalizedRequest {
    ...
    return request.copy(model = model, body = body.toString(), pricing = selected.modelConfig(request.preset).pricing?.at(java.time.Instant.now()))
}
...
internal suspend fun LLMServices.completeWithCommandCodeApi(
    ...
    repeat(maxToolRounds) { round ->
        ...
        val (status, response) = try {
            runCommandCodeUpstream(client, provider.commandCodeUrl, provider.apiToken, body,
                onRequest = { logUpstreamRequest("$source/commandcode-round-${round + 1}", it, provider, "commandcode"); debugPrompt(source, it) })
        ...
        if (calls.isEmpty()) return CommandCodeAttempt(status, sanitizeResponseBody(withUsage(response, usage), request.enableMarkdown))
        ...
        JsonParser.parseString(response).asJsonObject.getAsJsonArray("choices")[0].asJsonObject
            .getAsJsonObject("message").get("reasoning_content")?.let { assistant.add("reasoning_content", it.deepCopy()) }
        calls.forEach { call ->
            chat.getAsJsonArray("messages").add(JsonObject().apply {
                addProperty("role", "tool")
                addProperty("tool_call_id", call.id)
                addProperty("content", toolService.execute(call.name, call.arguments, requester.toolContext(request.currentUserText), COMMANDCODE_EXCLUDED_TOOLS))
            })
        }
    }
...
internal fun LLMServices.streamFromCommandCode(
    ...
    try {
        repeat(maxToolRounds) { round ->
            ...
            val (status, response) = try {
                ...
                if (round == 0 && assistantText.isEmpty() && provider.fallback != null) {
                    ...
                    emitAll(streamCommandCodeFallback(fallbackRequest, requester, requestId, "$source/fallback", fallback, quotaReservation))
                    return@flow
                }
                throw error
            }
            if (status !in 200..299) {
                if (shouldFallbackCommandCode(status, response, round, assistantText.isNotEmpty()) && provider.fallback != null) {
                    val fallback = provider.fallback.forMode(request.preset)
                    val fallbackRequest = commandCodeFallbackRequest(request, provider.fallback)
                    emitAll(streamCommandCodeFallback(fallbackRequest, requester, requestId, "$source/fallback", fallback, quotaReservation))
                    return@flow
                ...
                updateAccessRecord(requestId, "failed", errorMessage = response.take(512), groupName = requester.groupName, qqUid = requester.uid)
            ...
            if (calls.isEmpty()) {
                ...
                updateAccessRecord(requestId, "completed", usage, groupName = requester.groupName, qqUid = requester.uid)
                if (assistantText.isNotBlank()) recordConversationAnswer(requester, request.userContent, assistantText.toString(), provider)
                return@flow
            }
            appendAssistantToolCallMessage(chat.getAsJsonArray("messages"), response, calls)
            val assistant = chat.getAsJsonArray("messages").last().asJsonObject
        ...
        refundUsage(quotaReservation, usage, request, provider, requester.conversationId)
        updateAccessRecord(requestId, "failed", errorMessage = "Command Code tool round limit exceeded", groupName = requester.groupName, qqUid = requester.uid)
        emit(commandCodeError("Command Code tool round limit exceeded"))
    } catch (error: kotlinx.coroutines.CancellationException) {
        throw error
    } catch (error: Exception) {
        if (error !is QuotaSettlementException) refundUsage(quotaReservation, usage, request, provider, requester.conversationId)
        updateAccessRecord(requestId, "failed", errorMessage = error.message, groupName = requester.groupName, qqUid = requester.uid)
