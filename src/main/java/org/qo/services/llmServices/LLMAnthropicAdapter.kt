...
internal object LLMAnthropicAdapter : LLMAdapter {
	...
	fun fromChatRequest(
		...
		return JsonObject().apply {
			...
			chat.get("stop")?.takeIf { !it.isJsonNull }?.let { stop ->
				add(
					"stop_sequences",
					if (stop.isJsonArray) stop.deepCopy() else JsonArray().apply { add(stop.deepCopy()) })
			}
	...
	fun functionCalls(response: JsonObject): List<ResponseFunctionCall> =
		response.getAsJsonArray("content").mapNotNull { item ->
			val block = item.asJsonObject
	...
	private fun toolResult(id: String, output: String): JsonObject = JsonObject().apply {
		...
		if (runCatching { JsonParser.parseString(output).asJsonObject.has("error") }.getOrDefault(false)) addProperty(
			"is_error",
			true
		)
	}

	fun webSearchError(response: JsonObject): String? =
		response.getAsJsonArray("content").firstNotNullOfOrNull { item ->
			val block = item.asJsonObject
			if (block.get("type")?.asString != "web_search_tool_result") return@firstNotNullOfOrNull null
			val content =
				block.get("content")?.takeIf { it.isJsonObject }?.asJsonObject ?: return@firstNotNullOfOrNull null
	...
	fun toChatCompletion(response: JsonObject): String = JsonObject().apply {
		...
		add("choices", JsonArray().apply {
			add(JsonObject().apply {
				addProperty("index", 0)
				addProperty(
					"finish_reason", when (response.get("stop_reason")?.asString) {
						"max_tokens" -> "length"
						"tool_use" -> "tool_calls"
		...
		response.getAsJsonObject("usage")?.let { usage ->
			...
			add("usage", JsonObject().apply {
				...
				addProperty(
					"qapi_usage_complete", (usage.get("qapi_usage_complete")?.asBoolean ?: true) &&
...
internal class LLMAnthropicStreamState {
	...
	fun accept(event: JsonObject): AnthropicStreamUpdate {
		...
		if (type == "error") throw IllegalStateException(
			event.getAsJsonObject("error")?.get("message")?.asString ?: "Anthropic stream error"
		)
		...
		when (type) {
			...
			"content_block_delta" -> {
				...
				when (delta.get("type")?.asString) {
					...
					"thinking_delta" -> {
						block.addProperty(
							"thinking",
							block.get("thinking")?.asString.orEmpty() + delta.get("thinking").asString
						)
						return AnthropicStreamUpdate(phase = "thinking")
					}

					"signature_delta" -> block.addProperty(
						"signature",
						block.get("signature")?.asString.orEmpty() + delta.get("signature").asString
					)

					"input_json_delta" -> partialInputs.getOrPut(index) { StringBuilder() }
						.append(delta.get("partial_json").asString)
			...
			"message_stop" -> {
				...
				require(
					!current.get("stop_reason")?.takeIf { !it.isJsonNull }?.asString.isNullOrBlank()
				) { "Anthropic message has no stop reason" }
