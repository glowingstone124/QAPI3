...
internal suspend fun runAnthropicUpstream(
	...
	while (true) {
		...
		client.preparePost(url) {
			...
			if (request.get("stream")?.asBoolean == true) header(
				HttpHeaders.Accept,
			...
			if (response.contentType()?.match(ContentType.Text.EventStream) == true) {
				...
				flushEvent()
				completed =
					state.completed ?: throw IllegalStateException("Anthropic stream ended without message_stop")
			} else {
		...
		totalUsage.addProperty(
			"qapi_usage_complete", (totalUsage.get("qapi_usage_complete")?.asBoolean ?: true) &&
		...
		LLMAnthropicAdapter.webSearchError(result)
			?.let { return 502 to failure(anthropicError("web_search_error", it)) }
		...
		if (calls.isNotEmpty()) {
			...
			if (++pauses > 8) return 502 to failure(
				anthropicError(
					"search_round_limit",
					"Anthropic 搜索续接次数超过限制"
				)
			)
...
internal fun LLMServices.streamFromAnthropic(
	...
	try {
		...
		if (assistant.isNotBlank()) recordConversationAnswer(
			requester,
			request.userContent,
			assistant.toString(),
			provider
