...
class SearchChatHistoryTool(
	...
	override val definition = ToolSupport.functionTool(
		name = id,
		description = "检索当前群已经持久化的历史聊天。当用户询问较早讨论、某人以前说过什么、旧决定，或群事实摘要不足以理解接话和指代时使用。查询范围始终限制在当前群；query 留空时返回最新消息。",
		properties = linkedMapOf(
			"query" to ToolSupport.property(type = "string", description = "可选聊天内容关键词；留空时按时间返回最新消息。可与 uid、时间范围组合。"),
