...
- Web Search uses the SearXNG instance at `http://10.10.0.3:9123/` for every interactive request, including when other local tools are disabled. QAPI requests `/search?q=...&format=json` and gives the model up to eight results. Web conversations receive result URLs for citation. QQ and Minecraft conversations receive opaque result IDs instead of URLs and are instructed to answer without source attribution. `web_fetch` resolves a recent result from the same conversation, sends its URL internally to the planned Trafilatura service at `http://10.10.0.3:9124/fetch`, and returns up to 12,000 characters of page text without the URL for non-Web channels. Both tools work across Chat Completions, Responses, Anthropic, and Command Code. Enable `json` in SearXNG's `search.formats`; otherwise searches return HTTP 403. Chat Completions SSE waits for the tool rounds to finish before sending the final answer.
- Trafilatura microservice interface and deployment checks: [docs/WEB_FETCH_SERVICE.md](docs/WEB_FETCH_SERVICE.md). Until that service is deployed, `web_fetch` returns `fetch_unavailable`.
...
- `adminUids`: optional top-level array in the provider configuration listing QQ UIDs allowed to grant reset cards (default empty). It is reloaded together with the provider file; see [Reset cards](docs/RESET_CARDS.md).
